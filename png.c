...
/*TYPE **a is the plane you pass in.*/
int PlanePNG(int **a, int c)
{
  int i,j;
  char name[512];
  FILE *fp;
  ...
  /*c+a[][] is used here, this is the colouring loop, TYPE is just an integer ** , you may want to change this to have just int** 
  user col was defined when you called the colorRGB(), so that is ok.*/
  for (i=0; i < nRow; i++) {
    for (j=0; j < nCol; j++) {
      data[i*3*nCol + j*3] = userCol[c+a[i+1][j+1]][1];			//red
      data[i*3*nCol + j*3 + 1] = userCol[c+a[i+1][j+1]][2];		//green
      data[i*3*nCol + j*3 + 2] = userCol[c+a[i+1][j+1]][3];		//blue
    }
    ptr = data + i*3*nCol;
